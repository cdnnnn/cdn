...
import { Loader2, TrendingUp, Play, Plus, GitCompare, BookOpen, ChevronRight, Clock3, AlertCircle } from 'lucide-react';
              ...
              <div key={run.id} className={styles['dash__run-row']} onClick={() => navigate(`/app/history?id=${run.id}`)}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  {run.status === 'completed' ? (
                    ...
                    <div className={`${styles['dash__spinner']} ${styles['dash__spinner--failed']}`}><AlertCircle size={18} color="#DC2626" /></div>
                  ) : (
                ...
                <span className={`badge ${run.status === 'completed' ? 'badge-green' : run.status === 'failed' ? 'badge-red' : 'badge-run'}`}>{run.status}</span>
              </div>
            ))}
          </div>
...
.dash {
  ...
  &__spinner--failed { background: #FEE2E2; }
