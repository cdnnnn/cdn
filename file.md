...
.code__textarea {
  ...
  // No wrapping: both layers must break lines at exactly the same point
  // or the invisible cursor and the visible highlighted text drift apart
  // (typing/deleting then looks like it's happening "in the wrong
  // place"). Long lines scroll horizontally instead, via the outer
  // .code__editor-wrap's overflow: auto — same as most code editors.
  white-space: pre;
  word-break: normal;
