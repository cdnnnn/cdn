npm uninstall react-simple-code-editor

import { Component, ErrorInfo, KeyboardEvent, ReactNode, useEffect, useMemo, useRef, useState } from 'react';
...
// A small hand-rolled syntax-highlighted code editor — a transparent
// <textarea> (for real editing/cursor/selection) stacked on a highlighted
// <pre> (for the colors) underneath, kept in sync on scroll. Built
// directly on `prismjs` — a plain library, not a React component — so
// there's no third-party component whose module resolution can go wrong
// the way `react-simple-code-editor`'s did.
function CodeEditor({ value, onChange, placeholder }: { value: string; onChange: (v: string) => void; placeholder?: string }) {
  ...
  const highlighted = useMemo(() => {
    ...
    // A trailing newline keeps the pre's height in sync with the textarea
    // when the value itself ends in one.
    return `${html}\n`;
  }, [value]);
  ...
  // Tab inserts two spaces instead of moving focus, matching normal code
  // editor behavior.
  ...
  return (
    <div className={styles['code__stack']}>
      <pre ref={preRef} className={styles['code__pre']} aria-hidden="true">
...
// A render-time crash anywhere below (a bad third-party component, a
// malformed API response, etc.) would otherwise unmount the whole app and
// leave a blank screen. This boundary catches it and shows a recoverable
// message with a "Try again" reset instead.
interface ErrorBoundaryState { hasError: boolean; message: string }
class ErrorBoundary extends Component<{ children: ReactNode; fallbackLabel: string }, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    ...
    console.error(`[${this.props.fallbackLabel}] render error:`, error, info.componentStack);
  }
  render() {
    if (this.state.hasError) {
      return (
        <div className={styles.banner + ' ' + styles['banner--err']}>
          <AlertCircle size={15} />
          <span>
            {this.props.fallbackLabel} failed to load ({this.state.message}).{' '}
            <button type="button" className={styles['link-btn']} onClick={() => this.setState({ hasError: false, message: '' })}>Try again</button>
          </span>
        </div>
      );
    }
...
export default function CreateMetric(props: CreateMetricProps) {
  // Outer safety net — protects the whole page from any render crash
  // (not just the code editor), independent of where CreateMetric is
  // mounted from.
  return (
    <ErrorBoundary fallbackLabel="Metric builder">
      <CreateMetricInner {...props} />
    </ErrorBoundary>
  );
}

function CreateMetricInner({ onCancel, onSaved }: CreateMetricProps) {
  ...
  const handleEvalType = (t: EvalType) => {
    ...
    // Cleared here (not just inside the list-fetch effect) so the
    // "load code" effect — which also depends on evalType — doesn't fire
    // with this render's now-stale template name for the old eval type.
    setCodeTemplates([]); setSelectedCodeTemplateName('');
                      ...
                      <>
                        ...
                        {selectedCodeTemplateName && (
                          <div className={styles.code}>
                            ...
                            <div className={styles['code__editor-wrap']}>
                              <ErrorBoundary fallbackLabel="Code editor">
                                <CodeEditor value={code} onChange={setCode} placeholder="# scoring function" />
                              </ErrorBoundary>
                            </div>
                          </div>
                        )}
...
// The stack holds both layers at the same size/position: the highlighted
// <pre> underneath for color, a transparent <textarea> on top for actual
// editing. Height grows with content (min-height floors it); the
// scrolling container above is what caps it and scrolls.
.code__stack {
  position: relative;
  min-height: 520px;
}
...
// VS Code Dark+ inspired token colors. $ink-solid (the editor background)
// is a fixed dark surface regardless of the app's light/dark theme — same
// as .code__bar above — so these are fixed hex values on purpose rather
// than the theme's light/dark-flipping tokens: using a theme token like
// $card here was the earlier bug (it flips dark in dark mode, going
// dark-text-on-dark-background and becoming unreadable).
$vscode-fg:        #D4D4D4;
...
// The <pre> sits behind, purely visual (pointer-events off so clicks/typing
// go straight to the textarea above it).
.code__pre {
  position: relative;
  color: $vscode-fg;
  pointer-events: none;
  ...
  :global(.token.cdata) { color: $vscode-comment; font-style: italic; }

  :global(.token.keyword),
  :global(.token.boolean),
  :global(.token.constant),
  :global(.token.important) { color: $vscode-keyword; }

  :global(.token.operator),
  :global(.token.punctuation) { color: $vscode-punct; }

  :global(.token.string),
  :global(.token.char),
  :global(.token.triple-quoted-string) { color: $vscode-string; }

  :global(.token.number) { color: $vscode-number; }

  :global(.token.function),
  :global(.token.builtin) { color: $vscode-function; }

  :global(.token.class-name) { color: $vscode-class; }

  :global(.token.decorator),
  :global(.token.property),
  :global(.token.parameter) { color: $vscode-property; }
}

// The textarea sits on top, transparent, doing the real editing —
// perfectly overlaid on the <pre> below via matching absolute position,
// padding, font and line-height.
.code__textarea {
  position: absolute;
  inset: 0;
  width: 100%;
