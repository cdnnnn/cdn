...
export default function History() {
  ...
  // Opens the test-details drawer for a model — triggered by the details
  // icon in the results table. Metric scores no longer live in a separate
  // drawer view; they're shown directly in the results table now (see the
  // per-model metrics row below the table).
  const openDrawer = (model: ModelResult) => {
                            ...
                            {(results.results || []).map((r) => {
                              ...
                              return (
                                <Fragment key={r.model_id}>
                                  <tr className={`${r.rank === 1 ? styles.winner : ''} ${breakdown.length > 0 ? styles['row-has-metrics'] : ''}`}>
                                  ...
                                  {/* Per-metric breakdown, shown directly under the model's
                                      row rather than behind an icon/drawer — metrics are
                                      dynamic (name/count vary by run), so this renders
                                      whatever metric_breakdown actually contains. */}
                                  {breakdown.length > 0 && (
                                    <tr className={styles['metrics-row']}>
                                      <td colSpan={8}>
                                        <div className={styles['metrics-row__chips']}>
                                          {breakdown.map(([label, m]) => {
                                            const pct = Math.round(Math.max(0, Math.min(1, m.score ?? 0)) * 100);
                                            const tier = pct >= 80 ? 'good' : pct >= 50 ? 'mid' : 'low';
                                            return (
                                              <div key={label} className={styles['metric-chip']}>
                                                <div className={styles['metric-chip__top']}>
                                                  <span className={styles['metric-chip__label']}>{label}</span>
                                                  <span className={`${styles['metric-chip__score']} ${styles[`metric-chip__score--${tier}`]}`}>
                                                    {pct}%
                                                  </span>
                                                </div>
                                                <div className={styles['metric-chip__track']}>
                                                  <div
                                                    className={`${styles['metric-chip__fill']} ${styles[`metric-chip__fill--${tier}`]}`}
                                                    style={{ width: `${pct}%` }}
                                                  />
                                                </div>
                                                <div className={styles['metric-chip__counts']}>
                                                  {m.passed ?? 0}/{m.total ?? 0} passed
                                                </div>
                                              </div>
                                            );
                                          })}
...
// ---- per-model metric breakdown, shown inline in the results table -----
// (a sub-row under each model, not a separate drawer view — metrics are
// dynamic, so however many the run actually produced just wrap here)
.row-has-metrics td {
  border-bottom: none;
  padding-bottom: 8px;
}
