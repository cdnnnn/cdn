                    ...
                    (availableModels.length > 0 ? (
                      <div className={styles['ev__models-panel']}>
                        ...
                        {modelsLoading ? (
                          ...
                          // toggleModelForCategory / canGo above).
                          <div className={styles['ev__rag-columns']}>
                            {Array.from(filteredModelsByCategory.entries()).map(([category, categoryModels]) => {
                              const allCategoryIds = (modelsByCategory.get(category) ?? []).map((m) => m.id);
                              const selectedInCategory = categoryModels.find((m) => draft.models.includes(m.id));
                              return (
                            ...
                            {filteredModelsByCategory.size === 0 && (
                              <p className={styles.ev__empty}>No models match "{modelSearch}".</p>
                            )}
                          </div>
                        ) : (
