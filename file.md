...
const FilePanel = forwardRef<FilePanelHandle, FilePanelProps>(({ selectMode, onEnterSelectMode, onExitSelectMode, onDeleteComplete, active, onGoToInfer }, ref) => {
  ...
  const fetchFiles = useCallback(async (opts?: {
    ...
    try {
      ...
      dispatch(serverFilesFailure(err instanceof Error ? err.message : t('uploadInfer.filePanel.loadFilesFail')));
    } finally {
  ...
  const handleUpload = async () => {
    ...
    for (const bf of browsed) {
      ...
      try {
        ...
        setBrowsed(prev => prev.map(f => f.id === bf.id ? { ...f, status: ok ? 'success' : 'failed', error: ok ? undefined : t('uploadInfer.filePanel.uploadFail') } : f));
      } catch (err: any) {
        const apiMessage = err?.response?.data?.details?.result;
        const errorMsg = err?.response?.status === 403 && apiMessage
          ? apiMessage
          : err instanceof Error ? err.message : t('uploadInfer.filePanel.uploadFail');
            ...
            <div className={styles.paginationTopRow}>
              ...
              <div className={styles.pageNav}>
                <button
                  ...
                  aria-label={t('uploadInfer.filePanel.previousPage')}
                >
                ...
                <button
                  ...
                  aria-label={t('uploadInfer.filePanel.nextPage')}
                >
                  <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round">
                    <path d="M6 3l4 5-4 5" />
                  </svg>
        ...
        <div className={styles.dangerOverlay} onClick={() => setPromptViewer(null)}>
          <div className={styles.promptViewerModal} onClick={e => e.stopPropagation()}>
            <div className={styles.promptViewerHead}>
              ...
              <button className={styles.promptViewerClose} onClick={() => setPromptViewer(null)} aria-label={t('uploadInfer.filePanel.promptViewerClose')}>
                <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round">
                  <path d="M4 4l8 8M12 4l-8 8" />
                </svg>
              </button>
...
const FileSidebar: React.FC<Props> = ({ mode, active, activeFileId = null, onFileClick }) => {
  ...
  const fetchFiles = useCallback(async (opts?: {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('uploadInfer.filePanel.loadFilesFail'));
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className={styles.sidebar} data-tour={mode === 'select' ? 'infer-sidebar' : 'results-sidebar'}>
      ...
      <div className={styles.head}>
        ...
        {loading && <span className={styles.headSpinner} aria-label={t('uploadInfer.workspace.loadingFiles')} />}
                ...
                <>
                  ...
                  >
                    ...
                    <span className={styles.runBtnText}>
                      <span className={styles.runBtnTitle}>{t('uploadInfer.inferencePanel.runInference')}</span>
                ...
                <div className={styles.card}>
                    ...
                    <div className={styles.fg}>
                      <div className={styles.modelLabelRow}>
                        ...
                        >
                          ...
                          {t('uploadInfer.inferencePanel.refresh')}
                        </button>
                      </div>
                          ...
                          <div className={styles.flRow}>
                            ...
                            {n > 1 && (
                              <button
                                ...
                                title={summaryExpanded ? t('uploadInfer.inferencePanel.collapsePerFile') : t('uploadInfer.inferencePanel.viewPerFile')}
                              >
                          ...
                          <div className={styles.flRow}>
                            ...
                            {n > 1 && (
                              <button
                                ...
                                title={keywordExpanded ? t('uploadInfer.inferencePanel.collapsePerFile') : t('uploadInfer.inferencePanel.viewPerFile')}
                              >
                                <svg viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                                  <rect x="1.5" y="2" width="11" height="10" rx="2" />
                                  <path d="M4 5h6M4 7.5h4" />
                          ...
                          <div className={`${styles.perFilePanel} ${keywordExpanded ? styles.perFilePanelOpen : ''}`}>
                            <div className={styles.perFilePanelInner}>
                              {selectedFiles.map(f => (
                                <div key={f.id} className={styles.perFileRow}>
                                  <div className={styles.perFileName}>{f.original_name}</div>
                                  <div className={styles.perFilePrompt}>{f.keywords_prompt || <span className={styles.perFileEmpty}>{t('uploadInfer.inferencePanel.noPromptSet')}</span>}</div>
                                </div>
                              ))}
                            </div>
                          </div>
                          ...
                          <div className={styles.flRow}>
                            ...
                            {n > 1 && (
                              <button
                                ...
                                title={questionExpanded ? t('uploadInfer.inferencePanel.collapsePerFile') : t('uploadInfer.inferencePanel.viewPerFile')}
                              >
                                <svg viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                                  <rect x="1.5" y="2" width="11" height="10" rx="2" />
                                  <path d="M4 5h6M4 7.5h4" />
                          ...
                          <div className={`${styles.perFilePanel} ${questionExpanded ? styles.perFilePanelOpen : ''}`}>
                            <div className={styles.perFilePanelInner}>
                              {selectedFiles.map(f => (
                                <div key={f.id} className={styles.perFileRow}>
                                  <div className={styles.perFileName}>{f.original_name}</div>
                                  <div className={styles.perFilePrompt}>{f.faq_prompt || <span className={styles.perFileEmpty}>{t('uploadInfer.inferencePanel.noPromptSet')}</span>}</div>
                                </div>
                              ))}
                            </div>
                          </div>
                  ...
                  {n > 0 && (settings.generateSummary || settings.generateKeywords || settings.generateQuestions
                    ...
                    <div className={styles.promptActions}>
                      ...
                      >
                        {t('uploadInfer.inferencePanel.cancelPrompt')}
                      </button>
                      <button
                ...
                <div className={styles.batchSectionTitle}>
                  ...
                  {isBatchRunning && (
                    <span className={styles.liveStatus}>
                      ...
                      {t('uploadInfer.inferencePanel.live')}
                ...
                <div className={styles.batchColumns}>
                  ...
                  <div className={styles.batchCol}>
                    <div className={`${styles.batchColHead} ${styles.headQueued}`}>
                      ...
                      {t('uploadInfer.inferencePanel.queued')}
                  ...
                  <div className={styles.batchCol}>
                    <div className={`${styles.batchColHead} ${styles.headRunning}`}>
                      ...
                      {t('uploadInfer.inferencePanel.runningCol')}
                  ...
                  <div className={styles.batchCol}>
                    <div className={`${styles.batchColHead} ${styles.headCompleted}`}>
                      ...
                      {t('uploadInfer.inferencePanel.completed')}
...
}> = ({ data, loading, error, section, accent, onRename }) => {
    const { t } = useTranslation();
...
// ── Compare Inference: the slide-in drawer itself ────────
        ...
        <div className={`${styles.compareOverlay} ${open ? styles.compareOverlayOpen : ''}`} aria-hidden={!open}>
            ...
            <div className={`${styles.compareDrawer} ${open ? styles.compareDrawerOpen : ''}`} role="dialog" aria-modal="true">
                ...
                <div className={styles.compareBody}>
                    <CompareSide data={fromData} loading={fromLoading} error={fromError} section={section} accent="from" onRename={handleRename} />
                    <div className={styles.compareDivider} />
                    <CompareSide data={toData} loading={toLoading} error={toError} section={section} accent="to" onRename={handleRename} />
                </div>
            </div>
        </div>
    );
...

{
  ...
  "uploadInfer": {
    ...
    "workspacePanel": {
      ...
      "compareTitle": "Compare Inference Versions",
      "compareFrom": "Compare From",
      "compareTo": "Compare To",
      "compareLoadingVersions": "Loading versions…",
      "compareNeedTwo": "At least two inference versions are needed to compare.",
      "compareSelectPrompt": "Select a version above",
      "compareLoadFail": "Could not load this version.",
      "compareRenameBtn": "Rename version",
      "compareRenameEmpty": "Name can\u2019t be empty.",
      "compareRenameFail": "Could not rename this version.",
      "edit": "Edit",
      "copy": "Copy",
      "download": "Download",
      "copyAs": "Copy as",
...

{
  ...
  "uploadInfer": {
    ...
    "workspacePanel": {
      ...
      "close": "닫기",
      "compareBtn": "버전 비교",
      "compareTitle": "추론 버전 비교",
      "compareFrom": "비교 대상 (From)",
      "compareTo": "비교 대상 (To)",
      "compareLoadingVersions": "버전 불러오는 중…",
      "compareNeedTwo": "비교하려면 추론 버전이 2개 이상 필요합니다.",
      "compareSelectPrompt": "위에서 버전을 선택하세요",
      "compareLoadFail": "이 버전을 불러올 수 없습니다.",
      "compareRenameBtn": "버전 이름 변경",
      "compareRenameEmpty": "이름은 비워둘 수 없습니다.",
      "compareRenameFail": "이 버전의 이름을 변경할 수 없습니다.",
      "edit": "편집",
      "copy": "복사",
      "download": "다운로드",
      "copyAs": "형식으로 복사",
