                ...
                return (
                  ...
                  >
                    ...
                    {isRunning && (
                      ...
                      >
                        {cancelingId === e.id ? <Loader2 size={12} className={styles.spin} /> : <StopCircle size={12} />}
                        {cancelingId === e.id ? 'Stopping…' : 'Stop evaluation'}
                      </button>
                    )}
                  </div>
                );
...
.row__stop-btn {
  ...
  &:hover:not(:disabled) { background: $danger-wash; border-color: $danger; }
...
interface EvaluationsState {
  ...
  // card (spec: confirm dialog -> POST /evaluations/{id}/cancel).
...
export const cancelEvaluation = createAsyncThunk(
  ...
  async (evaluationId: string, { rejectWithValue }) => {
    try {
      const data = await evaluationsApi.cancel(evaluationId);
      return data;
    } catch (err) {
      const detail =
        (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail ||
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(cancelEvaluation.rejected, (state, action) => {
        state.cancelingId = null;
        const payload = action.payload as { evaluationId: string; message: string } | undefined;
        state.cancelError = payload?.message || 'Failed to stop evaluation';
      });
  },
});
...
export const evaluationsApi = {
  ...
  // Stops a running evaluation. Only meaningful while status === 'running'
  // (see the confirm dialog in History.tsx) — the backend responds 200 with
  // { status: 'cancelled', evaluation_id } on success.
