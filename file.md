...
export default function NewEvaluation() {
  ...
  // RAG-only: models grouped by category, one column per category (see
  // STEP 3 — MODELS below). Uses insertion order of first appearance so
  // the column order stays stable rather than alphabetizing on every
  // render. Two versions: the full set (availableModels) drives category
  // membership and the "exactly one per category" validation in canGo()
  // below, so a search query narrowing the visible list can't make that
  // check pass/fail incorrectly; the filtered version is what's actually
  // rendered in each column.
  const groupModelsByCategory = (list: Model[]) => {
  ...
  const modelsByCategory = useMemo(() => groupModelsByCategory(availableModels), [availableModels]);
  const filteredModelsByCategory = useMemo(() => groupModelsByCategory(filteredModels), [filteredModels]);
  ...
  const canGo = () => {
    ...
    if (step === 3) {
      // RAG: exactly one model per category is mandatory — not just "at
      // least one selected overall" (that's the Model/Agent rule below).
      // Every category present has to have exactly one of its models
      // chosen before continuing.
      if (draft.type === 'rag') {
        if (modelsByCategory.size === 0) return false;
        return Array.from(modelsByCategory.values()).every(
          (categoryModels) => categoryModels.filter((m) => draft.models.includes(m.id)).length === 1
        );
      }
                    ...
                    (availableModels.length > 0 ? (
                      <div className={styles['ev__models-panel']}>
                        ...
                        {modelsLoading ? (
                          ...
                          // RAG: one column per model category, exactly one
                          // selection required per column (see
                          // toggleModelForCategory / canGo above). Iterates
                          // over the FULL category set (modelsByCategory),
                          // not the search-filtered one — so a search that
                          // matches nothing in a given category still shows
                          // that category's (empty) column, rather than
                          // making it disappear and letting whichever
                          // column *does* have a match stretch to fill the
                          // whole row.
                          <div className={styles['ev__rag-columns']}>
                            {Array.from(modelsByCategory.entries()).map(([category, allCategoryModels]) => {
                              const categoryModels = filteredModelsByCategory.get(category) ?? [];
                              const allCategoryIds = allCategoryModels.map((m) => m.id);
                              const selectedInCategory = allCategoryModels.find((m) => draft.models.includes(m.id));
                              return (
                                <div key={category} className={styles['ev__rag-col']}>
                                  <div className={styles['ev__rag-col-head']}>
                                    ...
                                    <span
                                      className={`${styles['ev__rag-col-status']} ${
                                        selectedInCategory ? styles['ev__rag-col-status--on'] : ''
                                      }`}
                                    >
                                      {selectedInCategory ? (
                                        <>
                                          <Check size={11} strokeWidth={3} /> 1 selected
                                        </>
                                      ) : (
                                        'Select one'
                                      )}
                                  ...
                                  <div className={styles['ev__rag-col-list']}>
                                    {categoryModels.map((m) =>
                                      renderModelCard(m, () => toggleModelForCategory(m.id, allCategoryIds))
                                    )}
