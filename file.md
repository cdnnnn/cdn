npm install @dnd-kit/core @tiptap/react @tiptap/pm @tiptap/starter-kit @tiptap/extension-image @tiptap/extension-placeholder dompurify yet-another-react-lightbox
...
// ─────────────────────────────────────────────────────────────────────────
// Images render as small, fixed-size thumbnails instead of full width —
// dropping several in a row (or right after each other) reads as a simple
// grid rather than each one dominating its own line. No resize controls at
// all (an earlier version of this let you set an explicit width/height,
// which turned out to be an unreliable amount of complexity for something
// this doesn't need). Click a thumbnail to open it full-size in a zoomable
// lightbox (see TicketDescriptionEditor.tsx / TicketDetailSidebar.tsx); a
// small × button removes it from the description entirely while editing.
//
// `inline: true` (set where this is configured) is what lets thumbnails
// sit next to each other and wrap naturally instead of each forcing its
// own line, which is what actually produces the grid-like layout.
// ─────────────────────────────────────────────────────────────────────────

function GridImageView({ node, deleteNode, editor, extension }: NodeViewProps) {
  const { src, alt } = node.attrs as { src: string; alt?: string };
  const onImageClick = (extension.options as { onImageClick?: (src: string) => void })
    .onImageClick;

  return (
    <NodeViewWrapper as="span" className={styles['grid-image']}>
      <img
        src={src}
        alt={alt || ''}
        className={styles['grid-image__img']}
        onClick={() => onImageClick?.(src)}
      />
      {editor.isEditable && (
        <button
          type="button"
          className={styles['grid-image__remove']}
          onClick={(e) => {
            e.stopPropagation();
            deleteNode();
          }}
...
export const GridImage = Image.extend({
  addOptions() {
    return {
      ...this.parent?.(),
      // Set via .configure({ onImageClick }) by whoever mounts the editor
      // — this is how the NodeView (above) reaches back out to open that
      // component's own lightbox state without needing any global state.
      onImageClick: undefined as ((src: string) => void) | undefined,
    };
  },
  addNodeView() {
    return ReactNodeViewRenderer(GridImageView);
  },
});
...
import { GridImage } from './tiptapGridImage';
 ...
 *
 * Images render as small fixed-size thumbnails (tiptapGridImage.tsx) — drop
 * a few in and they lay out like a simple grid rather than each dominating
 * its own line. Click one to open it full-size in a zoomable lightbox; a ×
 * button on each thumbnail removes it while editing.
 */
export default function TicketDescriptionEditor({
  ...
  // toolbar actions, removing a thumbnail, etc). The sync effect below
  ...
  const editor = useEditor({
    extensions: [
      ...
      // allowBase64 is what lets pasted/dropped images embed directly as
      // data: URIs (the stock Image extension otherwise rejects them and
      // expects a hosted src). `inline: true` + the fixed thumbnail size in
      // GridImage's own CSS is what makes consecutive images wrap into a
      // grid-like row instead of each forcing its own line. onImageClick is
      // how GridImage's NodeView reaches back out to open the lightbox
      // below without needing any state outside this component.
      GridImage.configure({
        inline: true,
        allowBase64: true,
        onImageClick: (src: string) => setZoomSrc(src),
      }),
...
// Fixed small size, arranged as a lightweight "grid" purely via inline flow
// — consecutive images (whether pasted together or inserted one after
// another) wrap naturally into rows like a row of thumbnails, rather than
// each forcing its own full-width line. No resize controls; click a
// thumbnail to zoom (see the Lightbox in TicketDescriptionEditor.tsx), or
// remove it entirely with the × button.
.grid-image {
  position: relative;
...
.grid-image__remove {
  ...
  color: #fff;
  cursor: pointer;
  box-shadow: 0 1px 3px rgba(20, 22, 27, 0.3);
  transition: background 0.12s;
  &:hover {
    background: color-mix(in srgb, $danger 85%, black);
  }
}
...
// injected via dangerouslySetInnerHTML. `ADD_ATTR: ['style']` is cheap
// insurance rather than something load-bearing at this point — images no
// longer carry any size info at all (see tiptapGridImage.tsx: they're
// fixed-size thumbnails, no style/width/height persisted per-image), but
// forcing `style` into the allow-list explicitly, rather than trusting
// whatever DOMPurify's default config happens to permit, costs nothing and
// avoids re-discovering this the hard way if inline styles are ever used
// for something else in this content later.
...
types/tickets.ts	src/types/tickets.ts (or re-export from your types barrel)
api/tickets.ts	src/api/endpoints/tickets.ts
api/users.ts	src/api/endpoints/users.ts
store/ticketsSlice.ts	src/store/slices/ticketsSlice.ts
store/usersSlice.ts	src/store/slices/usersSlice.ts
components/ticketMeta.ts	src/components/tickets/ticketMeta.ts
components/TicketBoard.tsx	src/components/tickets/TicketBoard.tsx
components/TicketBoard.module.scss	src/components/tickets/TicketBoard.module.scss
components/TicketColumn.tsx	src/components/tickets/TicketColumn.tsx
components/TicketCard.tsx	src/components/tickets/TicketCard.tsx
components/TicketSelect.tsx	src/components/tickets/TicketSelect.tsx
components/TicketDescriptionEditor.tsx	src/components/tickets/TicketDescriptionEditor.tsx
components/TicketDescriptionEditor.module.scss	src/components/tickets/TicketDescriptionEditor.module.scss
components/CreateTicketDrawer.tsx	src/components/tickets/CreateTicketDrawer.tsx
components/CreateTicketDrawer.module.scss	src/components/tickets/CreateTicketDrawer.module.scss
components/TicketDetailSidebar.tsx	src/components/tickets/TicketDetailSidebar.tsx
components/TicketCloseConfirm.tsx	src/components/tickets/TicketCloseConfirm.tsx
store/store.ts	src/store/store.ts — merge into your existing file, don't overwrite it
store/hooks.ts	src/store/hooks.ts — only add if you don't already have typed useAppDispatch/useAppSelector
