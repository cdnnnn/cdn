// ═══════════════════════════════════════════════
// pages/SttTranscription/components/FileCard.tsx
//
// Two rendering modes:
//   - `variant="row"`  → flat row  (Processing section)
//   - `variant="card"` → grid tile (Library section)  ← default
// ═══════════════════════════════════════════════
import React from 'react';
import type { SttFileView } from '../../../store/sttSlice';
import { isVideo } from '../utils/srt';
...
interface Props {
    ...
    dimmed?: boolean;        // grayed out — not selectable (pipeline mode: pending files)
}

const IconVideo: React.FC = () => (
    <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        <rect x="1.5" y="3.5" width="10" height="9" rx="1.5" />
        <path d="M11.5 7l3-1.8v5.6L11.5 9z" />
    </svg>
);
...
const numericProgress = (p: SttFileView['progress']): number => {
...
const FileCard: React.FC<Props> = ({ file, onOpen, onGenerate, disableGenerate, variant = 'card', selected = false, selectionMode = false, dimmed = false }) => {
    const isCompleted = file.status === 'completed';
    const isFailed    = file.status === 'failed';
    const isPending   = file.status === 'pending';
    const isActive    = file.status === 'queued' || file.status === 'running';
    const canGenerate = isPending || isCompleted || isFailed;
    const showGenerate = canGenerate && !disableGenerate;

    const video    = isVideo(file.original_name);
    const progress = numericProgress(file.progress);

    // In selection mode, all non-processing cards are clickable (pending + completed + failed)
    const isSelectable = selectionMode && !isActive;
    const handleClick = () => {
        if (selectionMode && isSelectable) { onOpen(); return; }  // onOpen = toggleFileSelect in selectionMode
        if (isCompleted) onOpen();
    };

    // Status pill
    let pillClass = styles.pillGreen;
    let pillLabel = 'Completed';
    if (file.status === 'queued')  { pillClass = styles.pillAmber; pillLabel = 'Queued'; }
    if (file.status === 'pending') { pillClass = styles.pillGray;  pillLabel = 'Pending'; }
    if (file.status === 'running') { pillClass = styles.pillBlue;  pillLabel = 'Running'; }
    if (isFailed)                  { pillClass = styles.pillRed;   pillLabel = 'Failed'; }

    const generateLabel = isCompleted ? 'Re-generate' : isFailed ? 'Retry' : 'Generate';

    // ── Row variant (Processing section) ──────────────
    if (variant === 'row') {
        let barClass = styles.fileRowBarCompleted;
        if (file.status === 'queued')  barClass = styles.fileRowBarQueued;
        if (file.status === 'running') barClass = styles.fileRowBarRunning;
        if (file.status === 'failed')  barClass = styles.fileRowBarFailed;
        if (file.status === 'pending') barClass = styles.fileRowBarPending;

        return (
            <div
                className={`${styles.fileRow} ${barClass} ${isCompleted ? styles.fileRowClickable : ''}`}
                onClick={handleClick}
                role={isCompleted ? 'button' : undefined}
                tabIndex={isCompleted ? 0 : undefined}
                onKeyDown={e => { if (isCompleted && (e.key === 'Enter' || e.key === ' ')) { e.preventDefault(); onOpen(); } }}
            >
                <span className={`${styles.fileRowIcon} ${video ? styles.fileRowIconVideo : styles.fileRowIconAudio}`}>
                    {video ? <IconVideo /> : <IconAudio />}
                </span>
                <div className={styles.fileRowMain}>
                    <div className={styles.fileRowName} title={file.original_name}>{file.original_name}</div>
                    <div className={styles.fileRowSub}>#{file.id}</div>
                </div>
                ...
                {file.status === 'running' && (
                    <div className={styles.fileRowProgress}>
                        <div className={styles.fileRowProgressFill} style={{ width: `${Math.max(2, progress)}%` }} />
                    </div>
                )}
                {showGenerate && (
                    <button className={styles.fileRowAction} onClick={e => { e.stopPropagation(); onGenerate(); }} title={generateLabel}>
                        <IconGenerate />{generateLabel}
                    </button>
                )}
                {isCompleted && <span className={styles.fileRowChevron} aria-hidden="true"><IconChevron /></span>}
                {isActive && (
                    <>
                        <div className={styles.cardSweep} aria-hidden="true" />
                        <div className={styles.cardSweepMask} aria-hidden="true" />
                    </>
                )}
            </div>
        );
    ...
    // ── Card variant (Library grid — intrinsic-width row) ─
    let cardBarClass = styles.libCardBarCompleted;
    if (file.status === 'failed')  cardBarClass = styles.libCardBarFailed;
    if (file.status === 'pending') cardBarClass = styles.libCardBarPending;

    return (
        <div
            className={`${styles.libCard} ${cardBarClass} ${(isCompleted || isSelectable) ? styles.libCardClickable : ''} ${selected ? styles.libCardSelected : ''} ${selectionMode && !isActive ? styles.libCardSelectable : ''} ${dimmed ? styles.libCardDimmed : ''} ${file.add_to_lecture_pipeline === 1 ? styles.libCardInPipeline : ''}`}
            onClick={handleClick}
            role={(isCompleted || isSelectable) ? 'button' : undefined}
            tabIndex={(isCompleted || isSelectable) ? 0 : undefined}
            onKeyDown={e => { if ((isCompleted || isSelectable) && (e.key === 'Enter' || e.key === ' ')) { e.preventDefault(); handleClick(); } }}
        >
            {/* Checkbox — only when in selection mode and inference not yet submitted */}
            {isSelectable && (
                <span className={`${styles.libCardCheckbox} ${selected ? styles.libCardCheckboxChecked : ''}`}>
                    {selected && (
            ...
            {/* Icon badge */}
            <div className={`${styles.libCardIcon} ${video ? styles.libCardIconVideo : styles.libCardIconAudio}`}>
                <div className={styles.libCardIconGlyph}>
                    {video ? <IconVideo /> : <IconAudio />}
                </div>
            </div>

            {/* Filename + meta (id/type on one line, status + pipeline badge on the next) */}
            <div className={styles.libCardInfo}>
                <div className={styles.libCardName} title={file.original_name}>
                    {file.original_name}
                </div>
                <div className={styles.libCardMetaTop}>
                    <span>#{file.id}</span>
                    <span className={styles.libCardMetaSep}>·</span>
                    <span>{video ? 'Video' : 'Audio'}</span>
                </div>
                <div className={styles.libCardMetaBottom}>
                    <span className={`${styles.pill} ${pillClass}`}>
                        <span className={styles.pillDot} />
                        {pillLabel}
                    </span>
                    {showGenerate && (
                        <button
                            className={styles.libCardAction}
                ...
                {file.add_to_lecture_pipeline === 1 && (
                    <div className={styles.libCardPipelineLine}>
                        <span className={styles.lecturePipelineBadge} title="Added to Lecture Pipeline">
                            Lecture Pipelined
                        </span>
                    </div>
                )}
...
// Separate line beneath the status pill, holding only the pipeline badge —
// kept as its own flex row (rather than wrapping inside libCardMetaBottom)
// so it always starts a new line instead of just wrapping when space runs out.
.libCardPipelineLine {
  display: flex;
  align-items: center;
  margin-top: 4px;
...
// pipeline — now sits on its own line below the status pill, via
// .libCardPipelineLine, instead of wrapping inline next to it.
