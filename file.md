...
export default function NewEvaluation() {
  ...
  const canGo = () => {
    ...
    if (step === 5) {
      ...
      // Model-only: Retest on Wrong requires a verify metric to be chosen.
      if (draft.type === 'model' && draft.retestOnWrong && !draft.retestVerifyMetric) return false;
      return true;
    }
    return true;
  };
  ...
  const launch = async () => {
    ...
    if (draft.type === 'agent' && !draft.agentFramework) {
      ...
      const payload: CreateEvaluationRequest = {
        ...
        // Model-only — Metrics step's "Retest on Wrong" control. Always
        // sent when eval_type is 'model'; retest_max_rounds/
        // retest_verify_metric only added when retestOnWrong is true.
        // Omitted entirely for RAG (this branch also handles RAG).
        ...(draft.type === 'model'
          ? {
                  ...
                  {step === 5 && (
                    <div className={styles.ev__metrics}>
                      <div className={styles['ev__metrics-main']}>
                        ...
                        {/* Retest on Wrong — Model type only. Independent of
                            hideMetricsStep, since it's a separate control from
                            metric selection. */}
                        {draft.type === 'model' && (
                          <div className={styles['ev__retest']}>
                            <div className={styles['ev__retest-head']}>
                              <label className={styles.ev__label}>Retest on Wrong</label>
                              <div className={styles['ev__radio-row']}>
                                <button
                                  type="button"
                                  className={`${styles['ev__radio-opt']} ${
...
.ev {
  ...
  // ---- Retest on Wrong (Model type only, Metrics step) --------------------
  &__retest {
    margin-top: 18px;
    padding: 12px 14px;
    border: 1px solid $line;
...
export interface CreateEvaluationRequest {
  ...
  instruction?: string;
  // Model-only — Metrics step's "Retest on Wrong" control. Always sent for
  // eval_type 'model'; retest_max_rounds/retest_verify_metric are only
  // included when retest_on_wrong is true. Omitted entirely for Agent/RAG.
...
export interface EvaluationDraft {
  ...
  retestMaxRounds: number; // default 3, 1–10
