...
export default function HBar({ value, max = 100, color = '#1428A0', label, sublabel }: HBarProps) {
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      {sublabel && <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>{sublabel}</div>}
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ overflow: 'visible' }}>
      ...
      {metrics.map((m, i) => {
        ...
        return (
          <g key={i}>
            <line x1={cx} y1={cy} x2={lineEnd.x} y2={lineEnd.y} stroke="var(--border)" strokeWidth={1} strokeDasharray="3,3" />
            <text x={label.x} y={label.y} textAnchor="middle" dominantBaseline="middle" fill="var(--text-secondary)" fontSize={11} fontWeight={600} fontFamily="'Segoe UI', Roboto, Arial, sans-serif">
              {m}
            </text>
          </g>
        );
...
.toggle:hover { border-color: $indigo-light; }
.toggle:focus-visible { outline: none; box-shadow: 0 0 0 4px rgba(20, 40, 160, .12); }
...
.knob.dark { transform: translateX(28px); }
...
export default function ThemeToggle() {
  ...
  return (
    ...
    >
      <span className={`${styles.icon} ${!isDark ? styles.active : ''}`}><Sun size={13} /></span>
      <span className={`${styles.knob} ${isDark ? styles.dark : ''}`} />
      <span className={`${styles.icon} ${isDark ? styles.active : ''}`}><Moon size={13} /></span>
    </button>
  );
}
      ...
      <div className="pg-body">
        ...
        <div className={styles['comparison__grid']}>
          ...
          <div className="card" style={{ padding: 0 }}>
            <div className={styles['comparison__panel-title']} style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-light)' }}>Metric Breakdown</div>
            <table className="tbl">
          ...
          <div className={styles['comparison__scores']}>
            {comp.map((m, i) => (
              <div key={i} className={styles['comparison__score-item']}>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{m.ctx.toLocaleString()} ctx</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="pg-body">
        ...
        <div className={styles['dash__grid']}>
              ...
              <div key={run.id} className={styles['dash__run-row']} onClick={() => navigate(`/app/history?id=${run.id}`)}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{run.benchmark || '—'} &middot; {new Date(run.created_at).toLocaleDateString()}</div>
                  </div>
                </div>
          ...
          <div className="card">
            ...
            <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 12 }}>Strength comparison across 5 dimensions</div>
    ...
    <div className="page-enter pg-shell">
      ...
      <div className="pg-toolbar">
        <div className="toolbar">
          <div className="search-box"><Search size={16} color="var(--text-muted)" /><input placeholder="Search datasets…" value={search} onChange={(e) => setSearch(e.target.value)} /></div>
              ...
              <div className="card" key={b.name}>
                <div className="card-hdr">
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>tasks</div>
                  </div>
                </div>
    ...
    <div className="page-enter pg-shell">
                        ...
                        <div className={styles['suite-grid']}>
                          {filteredBenchmarks.map((b) => (
                            ...
                            >
                              ...
                              <div><span className="tag tag-amb">{b.type}</span><span style={{ fontSize: 11, color: 'var(--text-muted)', marginLeft: 8 }}>{b.task_count.toLocaleString()} tasks</span></div>
                            </div>
                          ))}
                        </div>
      ...
      {toast && (
        <div className="toast">
          ...
          <div><div style={{ fontWeight: 700, fontSize: 14 }}>Evaluation launched</div><div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Redirecting to History…</div></div>
        </div>
      )}
    </div>
  );
...
.sidebar {
  ...
  &__theme-row {
    ...
    padding: 6px 4px 14px; font-size: 12px; font-weight: 600; color: $text-secondary;
  }
  &__user {
...
export const STORAGE_KEY = 'semcoeval_theme';
...
function getInitialTheme(): Theme {
  ...
  // Defaults to light regardless of OS/browser preference, per requirement —
  // only an explicit, previously-persisted user toggle switches to dark.
...
body {
  ...
  // Smooth color transition when the theme toggle flips data-theme, rather
  // than every themed color snapping instantly.
