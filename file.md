...
export interface DatasetQuestionAnnotator {
  steps?: string | null;
  number_of_steps?: string | number | null;
  tools?: string | null;
}

export interface DatasetQuestionMetadata {
  level?: string | number | null;
  source?: string | null;
  annotator?: DatasetQuestionAnnotator | null;
}
...
export interface DatasetQuestion {
  ...
  // Present on some preview responses in place of / alongside
  // question_type — annotation-provenance info (difficulty level, source,
  // and who/what annotated it and how).
  metadata?: DatasetQuestionMetadata | null;
}
            ...
            <div key={q?.id ?? i} className={styles['datasets__preview-card']}>
              ...
              {q?.metadata && (
                <div className={styles['datasets__preview-meta']}>
                  {q.metadata.level != null && (
                    <div className={styles['datasets__preview-meta-item']}>
                      <span className={styles['datasets__preview-meta-label']}>Level</span>
                      <span className={styles['datasets__preview-meta-val']}>{q.metadata.level}</span>
                    </div>
                  )}
                  {q.metadata.source && (
                    <div className={styles['datasets__preview-meta-item']}>
                      <span className={styles['datasets__preview-meta-label']}>Source</span>
                      <span className={styles['datasets__preview-meta-val']}>{q.metadata.source}</span>
                    </div>
                  )}
                  {q.metadata.annotator?.number_of_steps != null && (
                    <div className={styles['datasets__preview-meta-item']}>
                      <span className={styles['datasets__preview-meta-label']}>Steps</span>
                      <span className={styles['datasets__preview-meta-val']}>{q.metadata.annotator.number_of_steps}</span>
                    </div>
                  )}
                  {q.metadata.annotator?.tools && (
                    <div className={styles['datasets__preview-meta-item']}>
                      <span className={styles['datasets__preview-meta-label']}>Tools</span>
                      <span className={styles['datasets__preview-meta-val']}>{q.metadata.annotator.tools}</span>
                    </div>
                  )}
                  {q.metadata.annotator?.steps && (
                    <div className={`${styles['datasets__preview-meta-item']} ${styles['datasets__preview-meta-item--full']}`}>
                      <span className={styles['datasets__preview-meta-label']}>Annotator notes</span>
                      <span className={styles['datasets__preview-meta-val']}>{q.metadata.annotator.steps}</span>
                    </div>
                  )}
                </div>
              )}
