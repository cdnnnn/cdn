...
// Handles both data: URIs (the common case — API returns base64 PNGs
// inline) and plain image URLs, downloading either as a real file rather
// than just opening it in a new tab.
...
const ZoomPanImage: React.FC<{ src: string; alt: string; onDownload?: () => void }> = ({ src, alt, onDownload }) => {
    const { t } = useTranslation();
    ...
    return (
        ...
        >
            ...
            <div className={styles.zoomControls}>
                ...
                {onDownload && (
                    <button type="button" className={styles.zoomControlBtn} onClick={onDownload} title={t('uploadInfer.workspacePanel.download')}>
                        <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
                            <path d="M8 2.5v8M4.8 7.3L8 10.5l3.2-3.2" /><path d="M3 13.5h10" />
                        </svg>
                    </button>
                )}
            </div>
...
const WordCloudView: React.FC<{ data: WordCloudData }> = ({ data }) => {
    ...
    const handleDownload = () => {
        const mimeMatch = data.wordcloud_image.match(/^data:image\/(\w+);/);
        const ext = mimeMatch ? mimeMatch[1] : 'png';
        const ts = new Date().toISOString().slice(0, 10);
        downloadImage(data.wordcloud_image, `wordcloud_${ts}.${ext}`);
    };
    return (
        <div className={styles.wordCloudImageWrap}>
            <ZoomPanImage src={data.wordcloud_image} alt={t('uploadInfer.workspacePanel.kiTabs.wordcloud')} onDownload={handleDownload} />
        </div>
    );
};
