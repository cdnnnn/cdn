...
// Elapsed time / ETA row on running cards — sits under the progress bar.
.statusTimeMeta {
  display: flex;
  align-items: center;
  gap: 10px;
...
const uploadSlice = createSlice({
  ...
  reducers: {
    ...
    updateFilePrompts(state, action: PayloadAction<{
      ...
      // Keep the Inference tab's own file list (selectFiles) in sync too —
      // it's a separate array from serverFiles, populated independently by
      // FileSidebar's own fetch, so patches above never reached it on their
      // own.
      state.selectFiles = patchBucket(state.selectFiles);
    },
    ...
    patchFileDictionaries(state, action: PayloadAction<Record<number, number | null>>) {
      ...
      state.selectFiles = state.selectFiles.map(apply);
    },
