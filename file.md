...
const PromptTemplates: React.FC = () => {
  ...
  // All fields are mandatory — used to disable the submit button live,
  // and re-checked on submit as a safety net.
  const isFormValid =
    form.name.trim() !== '' &&
                    ...
                    <tr key={tpl.id}>
                      ...
                      <td>
                        <div className={styles.rowActs}>
                          <button className={`${styles.btn} ${styles.btnSm}`} onClick={() => openEdit(tpl)}>
                            Edit
                          </button>
                          <button
...
// ═══════════════════════════════════════════════
// store/promptTemplateSlice.ts
// Content Analytics · Prompt Template CRUD
// ═══════════════════════════════════════════════
...
export interface PromptTemplateFormPayload {
  name: string;
  description: string;
  summary_prompt: string;
  keyword_prompt: string;
...
export interface UpdatePromptTemplatePayload extends PromptTemplateFormPayload {
  id: number;
}
...
export const fetchPromptTemplates = createAsyncThunk(
  'promptTemplate/fetchAll',
  async (_: void, { rejectWithValue }) => {
    try {
      const { data } = await api.get('/prompt_template/list_templates');
      return data as { total: number; templates: PromptTemplate[] };
    } catch (err: any) {
      return rejectWithValue(errMsg(err, 'Failed to load templates.'));
    }
  },
);
...
export const createPromptTemplate = createAsyncThunk(
  ...
  async (payload: PromptTemplateFormPayload, { rejectWithValue }) => {
    try {
      const { data } = await api.post('/prompt_template/create', payload);
      return data.result as PromptTemplate;
    } catch (err: any) {
      return rejectWithValue(errMsg(err, 'Failed to create template.'));
    }
  },
);
...
export const updatePromptTemplate = createAsyncThunk(
  ...
  async (payload: UpdatePromptTemplatePayload, { rejectWithValue }) => {
    try {
      const { data } = await api.post('/prompt_template/update_template', payload);
      // API returns the updated record directly (no `result` wrapper) per spec,
      // but we fall back gracefully if a wrapper is ever added server-side.
      return (data.result ?? data) as PromptTemplate;
    } catch (err: any) {
      return rejectWithValue(errMsg(err, 'Failed to update template.'));
    }
  },
);

export const deletePromptTemplate = createAsyncThunk(
  'promptTemplate/delete',
  async (id: number, { rejectWithValue }) => {
    try {
      await api.delete(`/prompt_template/${id}`);
      return id;
    } catch (err: any) {
      return rejectWithValue(errMsg(err, 'Failed to delete template.'));
    }
  },
);
...
const promptTemplateSlice = createSlice({
  ...
  initialState,
  reducers: {
    clearPromptTemplateError(state) {
      state.error = null;
    },
  },
  extraReducers: (builder) => {
    builder
      ...
      // create
      .addCase(createPromptTemplate.pending, (state) => {
        state.saving = true;
        state.error = null;
      })
      ...
      .addCase(createPromptTemplate.rejected, (state, action) => {
        state.saving = false;
        state.error = (action.payload as string) ?? 'Failed to create template.';
      })
      // update
      .addCase(updatePromptTemplate.pending, (state) => {
        state.saving = true;
        state.error = null;
      })
      ...
      .addCase(updatePromptTemplate.rejected, (state, action) => {
        state.saving = false;
        state.error = (action.payload as string) ?? 'Failed to update template.';
      })
      // delete
      .addCase(deletePromptTemplate.pending, (state, action) => {
      ...
      .addCase(deletePromptTemplate.rejected, (state, action) => {
        state.deletingId = null;
...
export const { clearPromptTemplateError } = promptTemplateSlice.actions;
