...
export default function NewEvaluation() {
  ...
  //
  // NOTE: assumes metricsSlice normalizes the response's `custom` array
  // into state.metrics.customMetrics, the same way it already does for
  // `metrics` -> state.metrics.allMetrics. If that slice hasn't been
  // updated yet, this reads as an empty array (fails safe: the section
  // just doesn't render) rather than throwing.
