...
export default function NewEvaluation() {
  ...
  // GET /providers — fetched once, the first time Step 2 is reached.
  useEffect(() => {
  ...
  // triggers one refetch the next time Step 4 is (re)entered.
  useEffect(() => {
  ...
  // metricsCatalog below) — `metrics` is ignored entirely.
  useEffect(() => {
                  ...
                  {step === 2 && (
                    <div className={styles.ev__scroll}>
                      <div className={styles['ev__step-toolbar']}>
                        ...
                        >
                          <RefreshCw size={14} className={providersRefreshing ? styles.ev__spin : ''} />
                        </button>
                      </div>
                      {providersRefreshing ? (
                    ...
                    (availableModels.length > 0 ? (
                      <div className={styles.ev__scroll}>
                        <div className={styles['ev__step-toolbar']}>
                          ...
                          >
                            <RefreshCw size={14} className={modelsRefreshing ? styles.ev__spin : ''} />
                          </button>
                        </div>
                        {modelsRefreshing ? (
                      ...
                      {datasetTab === 'browse' && (
                        <div className={styles.ev__suite}>
                          <div className={styles['ev__suite-scroll']}>
                            ...
                            {!datasetsLoading && !datasetsError && datasetsRefreshing && (
                              <div className={styles.ev__dgrid} aria-busy="true" aria-label="Refreshing test suites">
                                {Array.from({ length: SKELETON_CARD_COUNT }).map((_, i) => (
                      ...
                      {modelHidesMetrics ? (
                        ...
                        <div className={`${styles.ev__metrics} ${!requiresJudge ? styles['ev__metrics--single'] : ''}`}>
                          <div className={styles['ev__metrics-main']}>
                            ...
                            {metricsRefreshing ? (
                              <div className={styles.ev__chips} aria-busy="true" aria-label="Refreshing metrics">
                              ...
                              <div className={styles.ev__chips}>
                                ...
                                {!metricsLoading && metricsCatalog.length > 0 && filteredMetrics.length === 0 && (
...
.ev {
  ...
  &__samples-note { font-size: 0.75rem; color: $ink-3; padding-top: 34px; line-height: 1.4; max-width: 240px; }
...
// Native number input — browser spin buttons are hidden in favor of the
// custom +/- steppers above, which enforce the 1–20 clamp consistently.
.ev-preview-limit-input {
  width: 46px;
  text-align: center;
  border: 1.5px solid $line;
  border-radius: 8px;
