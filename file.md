...
function CodeEditor({ value, onChange, placeholder }: { value: string; onChange: (v: string) => void; placeholder?: string }) {
  ...
  // No manual newline compensation here: with white-space: pre (set on
  // .code__pre below), a trailing "\n" in the source already renders as
  // an empty final line, exactly like it does in the textarea — so the
  // two layers naturally stay the same height without any extra padding.
