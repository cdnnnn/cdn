                        ...
                        return (
                          <div className={styles['row__progress']}>
                            ...
                            <span className={styles['row__progress-label']}>
                              ...
                              {p && p.total > 0 ? ` · ${p.progress}/${p.total}` : ''}
                            </span>
                          </div>
                        );
                      })()}
...
// ---- live progress bar (running cards only) --------------------------------
.row__progress {
  margin-bottom: 8px;
}
.row__progress-track {
...
.row__progress-fill {
  height: 100%;
  border-radius: 999px;
  background: linear-gradient(90deg, $signal, $violet-ink);
...
// ---- metric column / grouped rows in the results table -----------------
// Each model can span multiple rows (one per metric, via rowSpan on the
// Rank/Model/Provider cells) rather than a single collapsed row.
.cell-metric {
  font-family: $font-mono;
  font-size: 0.9231em; // 0.75rem / 0.8125rem
  font-weight: 650;
...
// Rank/Model/Provider cells use rowSpan to merge across a model's metric
// rows — top-align them so they sit level with the first metric row
// instead of centering across the whole span.
...
export const evaluationsApi = {
  ...
  getProgress: (evaluationId: string) =>
    api.get<EvaluationProgressResponse>(`/evaluations/${evaluationId}/status`).then((r) => r.data),
...
interface EvaluationsState {
  ...
  // for rows whose status is 'running'; a card computes its own percentage
  // as (progress/total)*100 rather than storing it pre-computed, so it
  // stays correct if total is 0 momentarily (e.g. right as a run starts).
  progressByEvalId: Record<string, { progress: number; total: number; status: string }>;
}
...
export const fetchEvaluationProgress = createAsyncThunk(
  ...
  async (evaluationId: string) => {
    ...
    return { evaluationId, progress: data.progress, total: data.total, status: data.status };
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchEvaluationProgress.fulfilled, (state, action) => {
        const { evaluationId, progress, total, status } = action.payload;
        state.progressByEvalId[evaluationId] = { progress, total, status };
      });
  },
});
...
// GET /evaluations/{evaluation_id}/status — polled for 'running' rows only,
// right after each GET /evaluations fetch (initial load and the 10s
// background poll), so a running card can show live progress. Note: the
// path segment is named `dataset_id` in the API spec, but the id actually
// passed is the evaluation's own id.
...
export interface TestDetail {
  ...
  passed: boolean;
  // Per-metric score/pass-fail for this one test case — a single test case
  // can be scored against several metrics at once, so these are maps
  // keyed by metric name rather than a single "which metric" tag. Used to
  // scope the drawer's test-by-test list (and each card's shown pass/fail
  // state) when opened from a specific metric row in the results table.
  // Optional since not every response necessarily includes them.
  metric_scores?: Record<string, number>;
  metric_passed?: Record<string, boolean>;
}
// Per-metric pass/fail breakdown for one model — keys are metric names
// (e.g. "Answer Relevancy", "LLM JUDGE", "Exact Match"); which metrics
// appear depends entirely on what was run, so this is a dynamic map, not a
// fixed set of fields. `score` is a 0–1 fraction (not already ×100).
...
export interface ModelResult {
  ...
  total_tests: number;
  // Superseded by metric_breakdown below for display purposes (History no
  // longer has a dedicated "metric scores" view) — kept here since the API
  // may still send it, but the results table now reads metric_breakdown.
  metric_scores: Record<string, number>;
  metric_breakdown?: Record<string, MetricBreakdownEntry>;
  details: TestDetail[];
}
