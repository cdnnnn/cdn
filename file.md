        ...
        <div className={`${styles.verDropdownWrap} ${accent === 'from' ? styles.verDropdownFrom : styles.verDropdownTo}`} ref={ref}>
            ...
            >
                {selected ? (
                    ...
                    <span className={styles.verDropdownPlaceholder}>{versions.length === 0 ? '—' : 'Select a version'}</span>
                )}
...
const CompareSide: React.FC<{ data: InferenceVersionResult | null; loading: boolean; error: string | null }> = ({ data, loading, error }) => {
    const { t } = useTranslation();
    ...
    if (loading) return <div className={styles.compareSideState}><div className={styles.spinner} /></div>;
    if (error) return <div className={`${styles.compareSideState} ${styles.compareSideError}`}>{error}</div>;
    if (!data) return <div className={styles.compareSideState}>{t('uploadInfer.workspacePanel.compareSelectPrompt', 'Select a version above')}</div>;
    ...
    return (
        <div className={styles.compareSide}>
            ...
            <div className={styles.compareSection}>
                <div className={styles.compareSectionTitle}>{t('uploadInfer.workspacePanel.tabs.summary')}</div>
                {data.summary
            ...
            <div className={styles.compareSection}>
                <div className={styles.compareSectionTitle}>{t('uploadInfer.workspacePanel.tabs.keywords')}</div>
                <ChipGrid kws={data.keywords ?? []} />
            </div>

            <div className={styles.compareSection}>
                <div className={styles.compareSectionTitle}>{t('uploadInfer.workspacePanel.tabs.assessment')}</div>
            ...
            <div className={styles.compareSection}>
                <div className={styles.compareSectionTitle}>{t('uploadInfer.workspacePanel.tabs.shortAnswer')}</div>
            ...
            <div className={styles.compareSection}>
                <div className={styles.compareSectionTitle}>{t('uploadInfer.workspacePanel.tabs.trueFalse')}</div>
        ...
        <div className={`${styles.compareOverlay} ${open ? styles.compareOverlayOpen : ''}`} aria-hidden={!open}>
            ...
            <div className={`${styles.compareDrawer} ${open ? styles.compareDrawerOpen : ''}`} role="dialog" aria-modal="true">
                ...
                <div className={styles.compareBody}>
                    <CompareSide data={fromData} loading={fromLoading} error={fromError} />
                    <div className={styles.compareDivider} />
                    <CompareSide data={toData} loading={toLoading} error={toError} />
                </div>
            </div>
        </div>
    );
