                                ...
                                return breakdown.map(([metricLabel, m], idx) => (
                                  <tr key={`${r.model_id}-${metricLabel}`} className={winnerClass}>
                                    ...
                                    <td className={styles['cell-details']}>
                                      {(r.details?.some((d) => d.metric === metricLabel) ?? false) && (
                                        <button
                                          type="button"
        ...
        {detailsModel && (
          <>
            {(() => {
              ...
              // row (detailsMetric set); otherwise every test case for the
              // model (the no-breakdown fallback row).
              const scopedDetails = (detailsModel.details || []).filter(
                (d) => !detailsMetric || d.metric === detailsMetric
              );
              ...
              const passedCount = scopedDetails.filter((d) => d.passed).length;
...
export interface TestDetail {
  ...
  passed: boolean;
  // Which metric this test case belongs to — used to scope the drawer's
  // test-by-test list when opened from a specific metric row in the
  // results table (as opposed to a plain, non-metric-scoped model row).
  // Optional since not every response necessarily tags it.
  metric?: string;
}
