...
export default function NewEvaluation() {
  ...
  // Clamp + persist a single field of a selected model's retry config.
  // Clicks/keystrokes inside the config panel call e.stopPropagation() so
  // they don't also toggle the card's selection (see the card's onClick).
  const updateModelRetryConfig = (modelId: string, field: keyof ModelRetryConfig, raw: number) => {
    ...
    const clamped = Number.isNaN(raw) ? min : Math.min(max, Math.max(min, raw));
    dispatch(
      setDraft({
  ...
  // Same clamping, but for the single shared "apply to all" config used
  // when retryConfigMode === 'all'.
  ...
  const launch = async () => {
    ...
    if (draft.type === 'agent' && !draft.agentFramework) {
      ...
      // so it never got its own individual entry).
      const judgeRetryConfig: ModelRetryConfig =
      ...
      const payload: CreateEvaluationRequest = {
        ...
        ...(draft.retryConfigMode === 'all'
          ? { max_retries: draft.retryConfigAll.max_retries, timeout: draft.retryConfigAll.timeout }
          : {
              model_retry_config: draft.models.reduce<Record<string, ModelRetryConfig>>((acc, id) => {
                acc[id] = draft.modelRetryConfig[id] || { ...DEFAULT_MODEL_RETRY_CONFIG };
                return acc;
              }, {}),
            }),
                            ...
                            {draft.retryConfigMode === 'all' ? (
                              <div className={styles['ev__retry-mode-all']}>
                                ...
                                <p className={styles['ev__retry-mode-note']}>
                                  Applied to all {draft.models.length} selected model{draft.models.length === 1 ? '' : 's'}.
                                </p>
                              </div>
                            ) : (
                              <p className={styles['ev__retry-mode-note']}>
                                Set max retries and timeout on each selected model's card below.
                              </p>
                            )}
                          </div>
                        )}
...
.ev {
  ...
  // Custom metrics section sits directly below the built-in metrics chips,
  // inside the same scrolling column.
  &__custom-metrics {
    margin-top: 20px;
    padding-top: 18px;
    border-top: 1px dashed $line-2;
  ...
  // ---- instruction (optional, independent of metrics/judge gating) -------
  &__instruction {
    margin-top: 20px;
    padding-top: 18px;
    border-top: 1px dashed $line-2;
  }
  // When placed as the right-hand column of &__samples-instruction-row
  // (next to Run Samples) it's no longer stacked below another section,
  // so drop the top divider/spacing used in the old stacked layout.
  &__instruction--col {
    margin-top: 0;
    padding-top: 0;
    border-top: none;
...
export interface EvaluationDraft {
  ...
  retryConfigAll: ModelRetryConfig; // default { max_retries: 3, timeout: 300 }
...
const initialDraft: EvaluationDraft = {
  ...
  retryConfigAll: { max_retries: 3, timeout: 300 },
  modelRetryConfig: {},
  dataset: null,
  subgroup: [],
...
const evaluationsSlice = createSlice({
  ...
  reducers: {
    ...
    setDraftType(state, action: PayloadAction<EvaluationDraft['type']>) {
      ...
      state.draft.retryConfigAll = { max_retries: 3, timeout: 300 };
