...
.code__textarea {
  ...
  // Wrap, identically, in both layers. Not wrapping (an earlier attempt)
  // caused a worse bug: long lines then overflow horizontally, and a
  // textarea can't be scrolled to reach its own overflowed content the
  // same way the ancestor-scrolled <pre> can — so selection/cursor
  // mapping desyncs for anything past the visible edge (this is also why
  // "select all" looked incomplete). Wrapping removes horizontal overflow
  // entirely, so only vertical scroll (already synced below) is needed.
  // overflow-wrap (not word-break) is what real code-editor overlays use,
  // since browsers agree more consistently on where it breaks.
...
.code__textarea {
  ...
  min-height: 520px;
  resize: none;
  overflow: hidden; // never scroll independently of the .code__pre below it — that
                    // independent scroll is what causes the drift between the
                    // (invisible) real cursor and the (visible) highlighted text
  background: transparent;
  color: transparent;
