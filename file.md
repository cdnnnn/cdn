...
import type { FileResult, WordCloudData, ClustersData, FrequencyData, ImportanceComplexityData, ImportanceComplexityItem, GlossaryData, KeywordInsights } from './UploadInfer';
...
const ImportanceComplexityScatter: React.FC<{ data: ImportanceComplexityData }> = ({ data }) => {
    ...
    const W = 580, padL = 20, padR = 20, padT = 24, padB = 36, plotH = 260;
    const maxImportance = Math.max(1, ...items.map(it => it.importance));
    const maxFreq = Math.max(1, ...items.map(it => it.frequency));
    const colWidth = (W - padL - padR) / Math.max(1, complexities.length);
    const xForCol = (c: string) => {
        const idx = complexities.indexOf(c);
        return padL + colWidth * (idx + 0.5);
    };
    const yFor = (imp: number) => padT + plotH - (imp / maxImportance) * plotH;

    // ── Collision avoidance ──────────────────────────────────────────
    // Items with the same complexity + similar importance would otherwise
    // land on (or right next to) the exact same point. Within each
    // complexity column: sort by importance, then push any point that's
    // closer than MIN_GAP to the previous one further down, and jitter its
    // x position so overlapping dots fan out instead of stacking.
    const MIN_GAP = 34; // enough vertical room for both the dot and its label
    const positioned: (ImportanceComplexityItem & { x: number; y: number; r: number })[] = [];
    let maxY = padT + plotH;

    complexities.forEach(c => {
        const colItems = items
            .filter(it => it.complexity === c)
            .map(it => ({ it, y: yFor(it.importance) }))
            .sort((a, b) => a.y - b.y);

        let lastY = -Infinity;
        colItems.forEach(({ it, y }, idx) => {
            const adjY = Math.max(y, lastY + MIN_GAP);
            lastY = adjY;
            maxY = Math.max(maxY, adjY);
            const jitterSteps = [0, -1, 1, -2, 2, -3, 3];
            const jitter = jitterSteps[idx % jitterSteps.length] * Math.min(colWidth / 5, 14);
            const r = 5 + (it.frequency / maxFreq) * 9;
            positioned.push({ ...it, x: xForCol(c) + jitter, y: adjY, r });
        });
    });

    const H = Math.max(padT + plotH + padB, maxY + padB);

    return (
        <div className={styles.scatterWrap}>
            <svg viewBox={`0 0 ${W} ${H}`} className={styles.scatterSvg}>
                <line x1={padL} y1={padT + plotH} x2={W - padR} y2={padT + plotH} className={styles.scatterAxis} />
                <line x1={padL} y1={padT} x2={padL} y2={padT + plotH} className={styles.scatterAxis} />
                {complexities.map((c, i) => (
                    <text key={i} x={xForCol(c)} y={padT + plotH + 18} textAnchor="middle" className={styles.scatterAxisLabel}>{c}</text>
                ))}
                <text x={padL} y={padT - 8} className={styles.scatterAxisLabel}>{t('uploadInfer.workspacePanel.importanceAxis')}</text>
                {positioned.map((it, i) => (
                    <g key={i}>
                        <circle cx={it.x} cy={it.y} r={it.r} className={styles.scatterDot}>
                            <title>{`${it.keyword} — importance ${it.importance}, ${it.complexity}, freq ${it.frequency}`}</title>
                        </circle>
                        <text x={it.x} y={it.y - it.r - 5} textAnchor="middle" className={styles.scatterDotLabel}>{it.keyword}</text>
                    </g>
                ))}
            </svg>
        </div>
