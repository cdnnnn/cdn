...
import type { UploadDatasetParams, DatasetQuestion } from '../../api/endpoints/testSuites';
...
// GET /datasets/{dataset_id}/preview?limit=&offset= — paginated question
// preview. Takes the current page params as args so pagination controls in
// the drawer can just re-dispatch with a new offset/limit.
export const fetchDatasetPreview = createAsyncThunk(
  'testSuites/preview',
  async (
    { datasetId, limit, offset }: { datasetId: string; limit: number; offset: number },
    { rejectWithValue }
  ) => {
    try {
      return await testSuitesApi.preview(datasetId, limit, offset);
    } catch (err) {
...
const testSuitesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchDatasetPreview.pending, (state, action) => {
        state.preview.status = 'loading';
        state.preview.error = null;
        state.preview.datasetId = action.meta.arg.datasetId;
        state.preview.limit = action.meta.arg.limit;
        state.preview.offset = action.meta.arg.offset;
      })
      .addCase(fetchDatasetPreview.fulfilled, (state, action) => {
        // A later request could resolve out of order if the user paginates
        // quickly — ignore a stale response for a dataset/offset that's no
        // longer what's being viewed.
        if (
          state.preview.datasetId !== action.meta.arg.datasetId ||
          state.preview.offset !== action.meta.arg.offset ||
          state.preview.limit !== action.meta.arg.limit
        ...
        state.preview.questions = Array.isArray(action.payload?.questions) ? action.payload.questions : [];
        state.preview.total = typeof action.payload?.total === 'number' ? action.payload.total : 0;
      })
      .addCase(fetchDatasetPreview.rejected, (state, action) => {
        ...
        state.preview.error = (action.payload as string) || action.error.message || 'Failed to load dataset preview';
      });
  },
});

export const { resetUploadStatus, resetDeleteError, resetPreview } = testSuitesSlice.actions;
