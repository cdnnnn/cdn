...
const IconHelp: React.FC = () => (
    <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        <path d="M6.2 6.2a2 2 0 113.2 1.6L8 9" />
...
const FileLibrary: React.FC<Props> = ({ onOpen }) => {
    ...
    const tourSteps: TourStep[] = [
        {
            target: 'stt-title',
            title: 'Welcome to the STT Pipeline',
            content: 'Upload audio or video files, run AI transcription, edit captions, and manage your library — all in one place. This tour will walk you through the key features.',
            placement: 'bottom',
        },
        {
            target: 'stt-date-filter',
            title: 'Date Filter',
            content: 'Filter your library by date range. Set the From and To dates and click Apply to load files from a specific period. The Refresh button reloads the current range.',
            placement: 'bottom',
        },
        {
            target: 'stt-upload',
            title: 'Upload Files',
            content: 'Drag and drop audio or video files here, or use Browse Files / Browse Folder. Up to 5 files upload at once. Supported formats: MP3, WAV, AAC, FLAC, MP4, MOV and more.',
            placement: 'right',
        },
        {
            target: 'stt-library-heading',
            title: 'File Library',
            content: 'All your uploaded files appear here. Use the status chips to filter, the search icon to find files by name or ID.',
            placement: 'bottom',
        },
        {
            target: 'stt-status-chips',
            title: 'Status Filters',
            content: 'Filter files by status: All, Completed, Pending, Queued, or Running. Only Completed files can be opened to view and edit their transcript.',
            placement: 'bottom',
        },
        {
            target: 'stt-actions',
            title: 'Actions',
            content: 'Three actions available: ① Select files for Inference to run AI transcription, ② Delete Files to permanently remove selected files, ③ Move to Lecture Pipeline to send completed transcripts downstream.',
            placement: 'bottom',
        },
        {
            target: 'stt-library-body',
            title: 'File Cards',
            content: 'Each card shows the file name, ID, type, and status badge. A teal circle indicates a file is already in the Lecture Pipeline. Click any Completed card to open its transcript viewer and edit captions.',
            placement: 'top',
        },
    ];
            ...
            <div className={`${styles.libBody} ${showInferenceCol ? styles.libBodyInference : ''}`}>
                ...
                <div className={styles.libCol}>
                    ...
                    {panelMode === 'none' && !inferenceRunning && (
                        <div className={styles.actionToolbar} data-tour="stt-actions">
                            <button className={`${styles.actionToolbarBtn} ${styles.actionToolbarBtnInference}`} onClick={() => openPanel('inference')}>
                                <IconInference />
                                <span>{t('stt.library.selectForInference')}</span>
                            </button>
                            <button className={`${styles.actionToolbarBtn} ${styles.actionToolbarBtnDelete}`} onClick={() => openPanel('delete')}>
                                <IconDelete />
                                <span>{t('stt.library.deleteFiles')}</span>
                            </button>
                            <button className={`${styles.actionToolbarBtn} ${styles.actionToolbarBtnPipeline}`} onClick={() => openPanel('pipeline')}>
                                <IconPipeline />
...
// "?" help button that triggers the product tour
.tourHelpBtn {
  width: 28px;
  height: 28px;
  border-radius: 50%;
