...
// Shape returned by GET /models/health/{model_id}
...
export const modelsApi = {
  ...
  // GET /models/health/:modelId — checks whether an individual model is
  // currently reachable/healthy. Triggered manually by the user (not
  // automatically), returning a simple 'success' | 'failed' status derived
  // from the response body's `success` flag.
  checkHealth: (modelId: string) =>
    api
      .get<ModelHealthResponse>(`/models/health/${modelId}`)
      .then((r) => (r.data.success ? 'success' : 'failed')),
};
