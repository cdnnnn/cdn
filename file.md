...
// ─────────────────────────────────────────────────────────────────────────
// This component assumes the following slice changes exist (or need adding):
//
// metricsSlice:
...
//   - fetchMetrics is no longer called on mount; it's dispatched only once
//     the user picks a type in Step 2, and re-dispatched if they change it.
//
// modelsSlice:
//   - checkModelHealth(modelId: string) — GET /models/{id}/health
//     Response: { status: 'success' | 'failed' }
//   - state.models.healthById: Record<string, 'idle'|'loading'|'success'|'failed'>
//     keyed by model id, defaulting to 'idle' for models never checked.
// ─────────────────────────────────────────────────────────────────────────
...
const STAGE = [
  ...
  { title: 'Choose models', sub: 'Add every model you want scored side by side in this run. Only models that pass a health check can be selected.' },
...
export default function NewEvaluation() {
  ...
  const metricsList = (metricsState as any).metrics ?? (metricsState as any).allMetrics ?? [];
  ...
  // Kick off a health check for every candidate model as soon as it becomes
  // selectable (i.e. once its provider is chosen), so the badge is ready
  // (or at least "checking…") by the time the user reaches Step 3. Only
  // fires for models that haven't been checked yet.
  useEffect(() => {
    availableModels.forEach((m) => {
      const current = healthById?.[m.id];
      if (!current || current === 'idle') {
        dispatch(checkModelHealth(m.id));
      }
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // A model can only be added to the run once its health check has come
  // back successful — 'idle'/'loading'/'failed'/unknown are all blocked.
                            ...
                            return (
                              <button
                                ...
                                className={`${styles.ev__mcard} ${on ? styles['ev__mcard--on'] : ''} ${
                                  !selectable && !on ? styles['ev__mcard--disabled'] : ''
                                }`}
                                onClick={() => toggleModel(m.id)}
                                aria-disabled={!selectable && !on}
                                title={
                                  health === 'failed'
                                    ? 'Health check failed — this model cannot be selected'
                                    : health === 'loading' || health === 'idle'
                              ...
                              >
                                <div className={styles['ev__mcard-top']}>
                                  <div>
                                    ...
                                    <div className={styles['ev__mcard-provider']}>
                                ...
                                {/* Health badge — gates whether the card can be selected */}
                                <span
                                  className={styles['ev__health-badge']}
                                  data-health={health}
                                >
                                  {health === 'success' && (
                                    <>
                                  ...
                                  {(health === 'loading' || health === 'idle') && (
                                    <>
                                      <Loader2 size={12} className={styles.ev__spin} /> Checking health…
                                    </>
                                  )}
                                </span>
                      ...
                      <div className={styles['ev__metrics-main']}>
                        ...
                        <div className={styles['ev__metrics-bar']}>
                          <span className={styles['ev__metrics-count']}>
                            <b>{draft.selMetrics.length}</b> of {metricsList.length} selected
                          </span>
                          <div className={styles['ev__metrics-actions']}>
                            <button
                              type="button"
                              className={styles.ev__link}
                              onClick={() => dispatch(setDraft({ selMetrics: [...metricsList] }))}
                            >
                              Select all
                            </button>
