...
const FileLibrary: React.FC<Props> = ({ onOpen }) => {
    ...
    const tourSteps: TourStep[] = [
        ...
        { target: 'stt-actions',         title: t('stt.tour.actionsTitle'),         content: t('stt.tour.actionsBody'),         placement: 'bottom' },
...

{
  ...
  "stt": {
    "tour": {
      ...
      "actionsBody": "Three actions available: ① Select Files for Inference to run AI transcription, ② Delete Files to permanently remove selected files, ③ Move to Lecture Pipeline to send completed transcripts downstream.",
...

{
  ...
  "stt": {
    "tour": {
      ...
      "actionsTitle": "작업",
      "actionsBody": "세 가지 작업이 가능합니다: ① 추론을 위한 파일 선택 — AI 전사 실행, ② 파일 삭제 — 선택한 파일을 영구 삭제, ③ 강의 파이프라인으로 이동 — 완료된 전사본을 다음 단계로 전달.",
