...
const UploadInfer: React.FC = () => {
  ...
  const tourSteps: TourStep[] = [
    ...
    },
    {
      ...
      content: t('uploadInfer.tour.statusFilterBody', 'Show only files that are Waiting, Queued, Running, Inferenced, or Error \u2014 handy once you have a lot of files in flight.'),
      onEnter: () => setActiveTab('upload'),
    },
    {
      target: 'upload-actions',
      title: t('uploadInfer.tour.actionsTitle', 'Bulk actions, one click away'),
      content: t('uploadInfer.tour.actionsBody', 'Open this to associate a dictionary or a prompt template with your files, search by filename, export files, or delete them \u2014 all in bulk.'),
      placement: 'left',
      onEnter: () => setActiveTab('upload'),
    },
    {
...

{
  ...
  "uploadInfer": {
    ...
    "tour": {
      ...
      "statusFilterBody": "Show only files that are Waiting, Queued, Running, Inferenced, or Error — handy once you have a lot of files in flight.",
      "actionsTitle": "Bulk actions, one click away",
      "actionsBody": "Open this to associate a dictionary or a prompt template with your files, search by filename, export files, or delete them — all in bulk.",
...

{
  ...
  "uploadInfer": {
    ...
    "tour": {
      ...
      "statusFilterBody": "대기, 대기열, 실행 중, 추론 완료, 오류 상태의 파일만 골라서 볼 수 있습니다 — 처리 중인 파일이 많을 때 유용합니다.",
      "actionsTitle": "한 번의 클릭으로 실행하는 일괄 작업",
      "actionsBody": "여기를 열면 파일에 사전이나 프롬프트 템플릿을 연결하거나, 파일명으로 검색하거나, 파일을 내보내거나 삭제하는 작업을 한 번에 처리할 수 있습니다.",
