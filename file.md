...
// react-simple-code-editor is a CommonJS package; depending on the
// bundler's interop settings, a plain `import Editor from '...'` can
// resolve to the whole module namespace object instead of the component
// itself — which throws exactly the "expected a string... but got:
// object" crash. Resolve the actual component defensively so this works
// regardless of interop config.
// react-simple-code-editor is a CommonJS package; depending on the
// bundler's interop settings, a plain `import Editor from '...'` can
// resolve to the module namespace object instead of the component
// itself — sometimes wrapped in more than one layer of `.default`. Unwrap
// until we hit an actual function, and never trust the result blindly:
// if it's still not a function, EDITOR_COMPONENT stays null and the code
// section below falls back to a plain textarea instead of crashing.
function resolveDefaultExport(mod: unknown): unknown {
  let m = mod;
  let guard = 0;
  while (m && typeof m !== 'function' && typeof (m as { default?: unknown }).default !== 'undefined' && guard < 5) {
    m = (m as { default?: unknown }).default;
    guard += 1;
  }
  return m;
}
const EDITOR_COMPONENT = resolveDefaultExport(SimpleCodeEditor);
const Editor = (typeof EDITOR_COMPONENT === 'function' ? EDITOR_COMPONENT : null) as typeof import('react-simple-code-editor').default | null;
                              ...
                              {Editor ? (
                                <ErrorBoundary fallbackLabel="Code editor">
                                  <Editor
                                    ...
                                    padding={16}
                                    textareaId="scoring-code"
                                    className={styles['code__editor']}
                                    textareaClassName={styles['code__textarea']}
                                    preClassName={styles['code__pre']}
                                    placeholder="# scoring function"
                                  />
                                </ErrorBoundary>
                              ) : (
                                // The syntax-highlighted editor failed to load (bad
                                // module resolution) — fall back to a plain textarea
                                // so the user can still write/edit code.
                                <textarea
                                  className={styles['code__textarea-fallback']}
                                  spellCheck={false}
                                  value={code}
...
// Plain fallback when the syntax-highlighted editor fails to load — same
// look as before that library was added.
.code__textarea-fallback {
  display: block;
  width: 100%;
  min-height: 520px;
  border: none;
...
.code__pre {
  ...
  // Python syntax colors mapped to existing theme tokens instead of new
  // hex values — keyword/signal, string/ok, comment/muted, etc.
  :global(.token.comment) { color: $ink-3; font-style: italic; }
  :global(.token.keyword),
  :global(.token.operator) { color: $sky-ink; font-weight: 600; }
  :global(.token.string) { color: $ok; }
  :global(.token.number),
  :global(.token.boolean) { color: $amber-ink; }
  :global(.token.function) { color: $signal-2; }
  :global(.token.class-name),
  :global(.token.builtin) { color: $rose-ink; }
  :global(.token.decorator),
  :global(.token.punctuation) { color: rgba(255, 255, 255, 0.55); }
}
