...
export default function NewEvaluation() {
  ...
  // Only `metrics` from the API response is used for the built-in catalog
  // — it's the full list rendered as selectable chips. Loading state for
  // this step is tracked locally (metricsRefreshing) rather than read from
  ...
  // `custom` from the same response — richer objects (id/name/description/
  ...
  //
  // The exact key metricsSlice normalizes the response's `custom` array
  // into isn't confirmed, so this checks a few likely names (camelCase
  // `customMetrics`, and the raw response key `custom`, in case the slice
  // passes the payload through with only `metrics` renamed to `allMetrics`
  // and leaves `custom` untouched) before falling back to an empty array.
  const customMetricsRaw = (metricsState as any).customMetrics ?? (metricsState as any).custom ?? [];
  const customMetricsCatalog: CustomMetric[] = Array.isArray(customMetricsRaw) ? customMetricsRaw : [];
  ...
  const canGo = () => {
    ...
    if (step === 5) return Boolean(draft.judgeModelId);
    return true;
  };
