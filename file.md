...
// ── Actions — always-visible inline label + button row ──
.actionsInlineWrap {
  display: flex;
  align-items: center;
  gap: 6px;
...
.fcard {
  ...
  // Inset from the top/bottom + rounded on its own, so it reads as a clean
  // accent strip regardless of the card's overflow (the card no longer
  // clips its own content — see the .fcardSweep note below).
  &::before {
    content: '';
    position: absolute;
    left: 0;
...
// Shimmer sweep — a separate absolutely-positioned layer (not a pseudo-
// element on .fcard) so it can be clipped to the card's rounded shape
// independently, without putting overflow:hidden on .fcard itself (which
// was clipping real card content whenever it wrapped onto extra lines).
.fcardSweep {
  position: absolute;
  inset: 0;
