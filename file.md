...
export const api = {
  ...
  createSession: async (payload: CreateSessionPayload): Promise<ApiSession> => {
    try {
      ...
      // The create response has been observed to come back with an empty
      // db_ids right after creation (the server hasn't fully propagated the
      // link yet), even though the databases were sent in the request and
      // are properly attached moments later. Rather than show "0 databases"
      // until the next full refresh, trust the ids we just asked for as a
      // fallback whenever the response doesn't echo any back.
      const db_ids = normalized.db_ids.length > 0 ? normalized.db_ids : payload.db_ids;
      return { ...normalized, db_ids };
    } catch (err) {
