...
interface TicketCardProps {
   ...
   *  drag hook of its own, no menu/interaction chrome. */
  overlay?: boolean;
}
...
export default function TicketCard({
  ...
  // dnd-kit's activation distance means ordinary clicks (open card, menu
  // buttons) still fire normally — a drag only "activates" once the
  // pointer has moved a few pixels. Note: no live `transform` is applied
  // here — the source card stays put (just dimmed via isDragging); the
  // <DragOverlay> clone in TicketBoard is what actually follows the cursor.
...
import type { Ticket, TicketStatus, TicketResolution, TicketUser } from '../../types/tickets';
import { canTransition, OWNER_ONLY_HINT, type ColumnMeta } from './ticketMeta';
...
export default function TicketColumn({
  ...
  // Would the currently-dragged ticket be rejected if dropped here?
  const locked = !!activeTicket && isOver && !canTransition(activeTicket, col.status, currentUser.id);
