...
export type EvalType = 'llm' | 'agent' | 'rag';
...
export const testSuitesApi = {
  ...
  upload: async ({ file, name, description, evalType }: UploadDatasetParams): Promise<void> => {
    ...
    if (ext === JSONL_EXTENSION) {
      ...
      // contract; LLM and RAG uploads omit the field entirely.
...
export const { resetUploadStatus } = testSuitesSlice.actions;
...
function UploadModal({ uploadStatus, uploadError, onClose, onSubmit }: UploadModalProps) {
  const [evalType, setEvalType] = useState<EvalType>('llm');
